...
class ModelModuleBrainyFilter extends Model 
{
    ...
    public function cacheProductProperties($productIds = array())
    {
        ...
        if (!empty($productIds)) {
           ...
           $this->db->query('TRUNCATE ' . DB_PREFIX . self::TBL_FILTER);
        }
        ...
        foreach ($langs as $langId) {
            ...
            $sql->clean()
                ->select(array('quantity' => 'sum( IF(pov.quantity IS NULL, p.quantity, pov.quantity) )', 'p.product_id'))
                ->from(array('p' => 'product'))
            ...
            $sqlx->clean()
                ...
                ->set(array('f.quantity = t.quantity'));
