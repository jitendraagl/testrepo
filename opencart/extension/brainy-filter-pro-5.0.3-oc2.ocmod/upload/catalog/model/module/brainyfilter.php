...
final class ModelModuleBrainyFilter extends Model 
{
        ...
        foreach ($params as $param) {
            if (!empty($param)) {
                ...
                if ($pName === 'price') 
                {
                    ...
                    $price->inputMax = null;
                    if ((int)$p[0] > 0) {
                        $price->min = $this->currency->convert($p[0], $this->currency->getCode(), $this->config->get('config_currency'));
                        $price->inputMin = $p[0];
                    }
                    if ((int)$p[1] > 0) {
                        $price->max = $this->currency->convert($p[1], $this->currency->getCode(), $this->config->get('config_currency'));
                        $price->inputMax = $p[1];
                    }
     ...
     * and adds conditions for filtering by price if requested.
     * 
