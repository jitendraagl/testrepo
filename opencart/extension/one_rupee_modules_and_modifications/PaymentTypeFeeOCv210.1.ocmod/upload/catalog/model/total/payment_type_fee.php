...
class ModelTotalPaymentTypeFee extends Model {
	...
	public function getTotal(&$total_data, &$total, &$taxes) {
		...
		if ($this->config->get($classname . '_status') && $this->cart->getSubTotal()) {
			...
			//condition if cod charges
			if($payment_code == 'cod'){
				$cod_charge_product_wise = 0;
				
				// getting shipping cost for each product in cart
				foreach ($this->cart->getProducts() as $result) {
				
					$prodShipping = $this->db->query("select cod_charges from ". DB_PREFIX."product where product_id = " . $result['product_id']);		//Bug Fix By jitendra gaur jitengaur@gmail.com
					$cod_charge_product_wise += (double)$prodShipping->row['cod_charges'];

				}
				//if cod product wise is empty then default value will be apply
				if($cod_charge_product_wise){
					$value = $cod_charge_product_wise;
				}
			}
